...
async function renderTracks() {
  ...
  var pricingRows = EXAM_TYPES.map(function (t) {
    ...
    // A non-required track (no real state exam -- Exam Qs/Duration/etc. are just this site's own
    // self-set practice benchmark) is grayed out uniformly, regardless of its bank-depth % or
    // category, per the user's 2026-09-13 request -- the red/green bank-health highlighting below
    // is about real exam risk, which doesn't apply to a track with no real exam to be at risk for.
    // Bold red is reserved for the actual risk case: a thin practice bank (>20% of the pool drawn
    // per sitting, i.e. below the project's 5:1 pool-to-draw aspirational target) on a real,
    // state-required exam (2026-09-12: raised from the prior 4:1/25% hard-floor threshold to the
    // 5:1 aspirational one, per user request). The green "healthy bank" cutoff (<=25%, on the
    // older 4:1 threshold) still applies to required tracks only now that non-required ones are
    // grayed out instead.
    var bankPctRowClass = !examRequired ? 'settings-track-not-required'
      : (bankPct === null ? '' : (bankPct > 20 ? 'settings-bankpct-high' : (bankPct <= 25 ? 'settings-bankpct-low' : '')));
...
var VISITORS_DEFAULT_FILTERS = { preset: 'today', country: '', countryOp: 'eq', region: '', regionOp: 'eq', minDurationSec: '60', maxDurationSec: '', minPages: '', maxPages: '', reachedBuyOnly: false, excludeBots: true };
...
var VISITORS_DATE_PRESETS = [['all', 'All Time'], ['today', 'Today'], ['yesterday', 'Yesterday'], ['7d', 'Last 7 Days'], ['30d', 'Last 30 Days'], ['custom', 'Custom Range']];
...
function visitorsDateRangeForPreset(preset) {
  ...
  if (preset === 'yesterday') {
    ...
    return { from: todayStart - 86400, to: todayStart - 1 };
  }
...
function visitorsQueryString() {
  ...
  var excludeBotsCheckbox = document.getElementById('visitors-exclude-bots-checkbox');
  ...
  // Client-side only (checked against the already-loaded pages_json / is_bot flag, see
  // loadVisitors()) -- doesn't become a query param, just read here alongside the rest for the
  // same "Apply" flow.
  visitorsFilters.reachedBuyOnly = reachedBuyCheckbox ? reachedBuyCheckbox.checked : false;
  visitorsFilters.excludeBots = excludeBotsCheckbox ? excludeBotsCheckbox.checked : false;
...
function drawVisitorsTable() {
  ...
  var filteredCache = visitorsCache;
  if (visitorsFilters.reachedBuyOnly) filteredCache = filteredCache.filter(function (v) { return v.reachedBuy; });
  if (visitorsFilters.excludeBots) filteredCache = filteredCache.filter(function (v) { return !v.is_bot; });
